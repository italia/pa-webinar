...
export default async function middleware(request: NextRequest) {
  ...
  // Every /admin/* page (except /admin/login, handled above) requires a VALID
  // admin session — verified cryptographically (signature + expiry). A
  // UUID-shaped `?token=` on /admin/events/{id}[/edit] used to bypass this,
  // which left an admin whose session had expired still able to view and save
  // event settings via the event moderator token (nav gone, page still worked).
  // The admin event edit/detail links carry `?token=` only so the save PUT can
  // authenticate; those pages are generated by the admin panel, never sent to
  // moderators (their email links to the public event page + live room), so
  // gating them on the admin session is correct and closes the hole. An expired
  // session now redirects to /admin/login everywhere under /admin.
