...
| TURN relay for restrictive networks | **Not included**: browsers must reach UDP port 10000 directly | Optional coturn from the Jitsi subchart, off by default (`jitsi-meet.coturn.enabled`; keys in [Deploying with Helm](DEPLOYMENT.md#coturn-turn-and-turns), when TURN is needed in [Infrastructure](INFRASTRUCTURE.md#turn)); on one k3s server, `pa-webinar-up.sh --turn`, with TURN over TLS on the portal's port 443 |
...
The Compose stack is for development, not for evaluating an installation or running events: the portal is served over plain HTTP, the secrets are public placeholders and no container has resource limits. To try the product and the chart on a workstation, use [minikube](install/minikube.md). To run PA Webinar on a single machine, `infra/onprem/k3s/pa-webinar-up.sh` installs k3s and the Helm chart on one server with one command: a supported small-production layout, without high availability ([Installing on your own VMs with k3s](install/k3s.md)).
...
To see a chart change running, install it on minikube with `scripts/minikube-up.sh --images local` ([Try PA Webinar on minikube](install/minikube.md)). Always pass `--images local` to see your own changes: with registry credentials in the environment, or in the `ghcr-pull` Secret an earlier run left in the namespace, the automatic choice installs the published images instead, and the script warns about it. In local mode the script builds the portal and migration images from your checkout, uncommitted changes included, tags them per profile (`pa-webinar:local-<profile>`), and loads them into the node; the migration image is loaded again only when the Dockerfile, the lockfiles or `app/prisma` change. `/api/health` then answers `"version":"local"` and the commit you built, with `-dirty` for uncommitted changes. The build log is `build.log` in the profile's state folder, and every phase prints its duration.

Measured with Docker's build cache warm, a one-string change in the portal took 2 min 21 s from the edit to the running pod: 1 min 50 s of build, the application image loaded, 21 s of Helm. That is why Compose stays the faster loop for application code, and minikube the place to test the chart, its values and the Kubernetes jobs. `--values FILE` layers a chart option you are trying (for example `backup.enabled: true`) after the generated values, and `--profile` runs a second installation next to the first ([More than one profile](install/minikube.md#more-than-one-profile)). The loop, step by step, is in [The development loop](install/minikube.md#the-development-loop), and the installation check in [Check it yourself](install/minikube.md#check-it-yourself).
