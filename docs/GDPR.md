...
| **Q&A panel** questions and upvotes | `Question`, `QuestionUpvote` | Plain text: author name and question | Event retention | GDPR cleanup. Erasure removes a registrant's own questions |
...
| **Materials** | `EventMaterial` and its files | Files in object storage. The visibility setting (**Always**, **Before the event**, **During the event**, **After the event**) decides which public lists show a material, not who can download it: an uploaded file is served from its `/api/assets/…` URL to anyone who has the URL. **Always**, the default, lists a material on the public event page from publication | Event retention | GDPR cleanup, rows and files |
...
| Docker Compose schedules only three jobs | Address-book retention, the orphan sweep and the track purges do not run. With the `recorder` profile, per-participant audio is never deleted by a job | Schedule `/api/cron/rubrica-retention` in the host's cron, and with the `recorder` profile the purge routes too ([how](architecture/background-jobs.md#docker-compose-on-a-single-vm)) |
