...
| A daily job deletes event-scoped personal data once an event's retention period has passed, whether or not the event was ended | Keep the scheduled jobs running: the cleanup job, and the lifecycle job or the JVB scaler that end events on time (see [Retention and cleanup](#retention-and-cleanup)) |
...
**Event retention** means: after the event's `endsAt` plus its `dataRetentionDays`. For an event that was never ended, the clock starts at the later of `endsAt` and its last activity. Drafts are never cleaned. The daily GDPR cleanup (`/api/cron/cleanup`) applies it. See [Retention and cleanup](#retention-and-cleanup).
...
sequenceDiagram
  ...
  alt right of access, on /privacy/my-data
    ...
    API->>API: verify signature, action and age
    API->>DB: delete registrations and the address-book entry by emailHash
...
Erasure deletes every `Registration` with the requester's email hash. The database cascade removes the person's Q&A questions and upvotes, poll votes, agenda reactions and reminder bookkeeping with it. It also deletes the address-book entry (`Person`) with the same email hash, even when no registration is left; the response reports it as `addressBookDeleted`, and the confirmation page says so.
...
- invitations and named grants, including those linked to the deleted address-book entry, which lose the link;
...
- **Opt-out.** `/rubrica/opt-out?token=…` and `POST /api/rubrica/opt-out` accept a token signed with `APP_SECRET`, valid for 90 days (`app/src/lib/persons/opt-out-token.ts`). The page shows the name and organization, and the button marks the entry opted out; the daily job then deletes it. For registrants in the address book, the confirmation, reminder and post-event thank-you emails carry this link, with a footer note on how to leave the address book, also when an administrator has customized the footer (`app/src/lib/persons/opt-out-link.ts`).
...
|---|---|---|
| The email outbox is never purged | Sent emails stay in `EmailOutbox`, encrypted, including personal links. Calendar attachments, with the event contact's name, are in plain text | Purge old `SENT` rows periodically at database level. Keep the `moderator-link` rows: they record which moderator links were already sent, and deleting them lets a later publication send the primary link again |
