...
- **Role enforcement in the conference depends on the Jitsi wiring.** The chart and Docker Compose wire token-based roles by default. An external Jitsi, or values that replace the chart's Prosody modules or volumes, must provide the same wiring, and on Compose a participant can hold the moderator role for a moment after joining ([Server-side role enforcement](jitsi-integration.md#server-side-role-enforcement)).
